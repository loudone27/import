...
2°) Kibana  
2.1°) Preparation  
  2.1.1°) Download the targz package 6.8.7  
    - kibana-6.8.7.tar.gz  

  2.1.2°) Node Elasticsearch Home test information  
    Node1 => 192.168.1.51  
    Node2 => 192.168.1.52  
    Node3 => 192.168.1.53  

  2.1.3°) Find your correct Plugin version
 Kenobee need the following kibana plugin in order to have it for the new kibana instance   
  
[Kibana_health_metric_vis](https://github.com/clamarque/kibana_health_metric_vis/releases)  
[logtrail](https://github.com/sivasamyk/logtrail/releases)  
[Sentinl](https://github.com/sirensolutions/sentinl/releases)  

2.2°) Migration Steps  
  For this step you can migrate kibana instance as you want, there are not there is no particular constraint, 
  all you need is to stop every kibana instances of the specific kenobee Cluster.
  
  2.1.1°) Stop_Kibana_instance
  
  In order to stop node go here:  

  2.2.1°) Save actual node  
  In order to be able to rollback it is important to save the current binary directory of the node  
  For That, please follow the following step below:  
  ...
  ```bash
    ...
    sudo -u elasticsearch -i
    cd /SERVICE/run/
    tar -zcvf Kibana_instance.tar.gz Kibana_instance
  ```

&#x1F538; **Please note:** You need to replace **Kibana_instance.tar.gz** & **Kibana_instance** by the correct kibana folder instance  like below:  

  - Kibana instance:  **kibana-6.5.4**  / **kibana-6.5.4.tar.gz**

  2.2.2°) Save actual configuration files
  In order to be able to keep actual configuration it is important to save the current configurations file of the different nodes
  For That, please follow the following step below:

  **If you don't already have the save Folder SAVE_CONF**
  ...
  ```bash
    ...
    mkdir -m 755 SAVE_CONF  
    cd SAVE_CONF  
    mkdir -m 755 kibana_instance  
    
    cd /SERVICE/run/kibana_folder/config  
    cp kibana.yml /SERVICE/run/SAVE_CONF/kibana_instance  
    
    cd /SERVICE/run/kibana_folder/plugins/logtrail
    cp logstash.json /SERVICE/run/SAVE_CONF/kibana_instance  
    
    mv Kibana_instance.tar.gz /SERVICE/run/SAVE_CONF/kibana_instance  
    
  ```

  **If you already have the save Folder SAVE_CONF**
  
  ```bash
    With the user elasticsearch  
    sudo -u elasticsearch -i  
    
    cd /SERVICE/run/SAVE_CONF  
    mkdir -m 755 kibana_instance  
...
&#x1F538; **Please note:** You need to replace **kibana_folder** & **Kibana_instance.tar.gz** by the correct kibana folder instance like below:  

  - Kibana instance:  **kibana-6.5.4**  / **kibana-6.5.4.tar.gz**

  2.2.3°) Add the new version package  
  For this we need to paste on the kenobee server the elactisearch tar.gz package for the migration kibana instance  
  Please following steps:

  **If you don't already have the package Folder PACKAGE**
  ...
  cd /SERVICE/run/PACKAGE
  mkdir -m 755 kibana_plugin
  ```

  And then use sftp tool (ex winscp) for copy kibana package in the folder **/SERVICE/run/PACKAGES**  
  And copy kibana plugin package in the folder **/SERVICE/run/PACKAGES/kibana_plugin**  
  
  **If you already have the package Folder PACKAGE**  
  ...
  Use sftp tool (ex winscp) for copy kibana package in the folder **/SERVICE/run/PACKAGES**  
  And copy kibana plugin package in the folder **/SERVICE/run/PACKAGES/kibana_plugin**  

  2.2.3°) Untar Package  
  For this step, you will create the new binary folder for the new kibana instance  
  Please follow these steps:  
  ...
  cd /SERVICE/run/PACKAGES  
  tar xvzf kibana-6.8.7.tar.gz -C /SERVICE/run/
  
  ```

  2.2.4°) new plugin installation  
  For this step please follow those instruction here : 
  you will find all instruction for the plugin installation below:
  
  - logtrail
  - sentinl
  - health_metric_vis

  2.2.5°) Add kibana configuration to the new kibana instance  
  This Step, you will paste configuration files from the previous kibana instance to the new one  

  ```bash
  With the user elasticsearch  
  sudo -u elasticsearch -i  
  
  cd /SERVICE/run/SAVE_CONF/kibana_instance  
  cp kibana.yml /SERVICE/run/new_kibana_folder/config  
  cp logtrail.json /SERVICE/run/new_kibana_folder/plugins/logtrail
  
  ```

&#x1F538; **Please note:** You need to replace **new_kibana_folder** by the correct new kibana folder like below:  

  - kibana_instance:    **Folder: kibana-6.8.7**  

  2.2.6°) Modify the Symbolic Links Destination kibana  
  This step consist of changing the Symbolic Links and point it to the new kibana's binary folder  
  ...
  rm -f kibana
  ln -s /SERVICE/run/new_kibana_folder kibana
  ```

&#x1F538; **Please note:** You need to replace **new_kibana_folder** by the correct kibana folder instance like below:  

  - new_kibana_folder:    **Folder: kibana-6.8.7**  
